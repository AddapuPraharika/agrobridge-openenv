...
[![Running on HF](https://img.shields.io/badge/HuggingFace-Running-green)](https://praharika18-agrobridge-openenv.hf.space)
[![OpenEnv Spec](https://img.shields.io/badge/OpenEnv-Compliant-blue)](https://huggingface.co/spaces/praharika18/agrobridge-openenv)
...
| `/step` | POST | Assign a farmer — returns reward + next obs |
...
### Example
A **senior** spraying farmer assigned to **hard** pesticide spraying with **HIGH urgency**:
```
...
A **junior** cotton farmer assigned to the same job:
```
base = 0.0 (no match)
...
Farmer **availability is randomized** each episode (70% chance available) — the agent must work with whoever is present.

---

## 🔄 Multi-Step Episode Design

Each episode runs up to **3 steps**. The episode ends early if:
- The agent achieves a perfect reward (`≥ 1.0`) — success
- All 3 steps are exhausted — evaluated on cumulative reward

This forces the agent to **learn from suboptimal assignments** and try again — genuine RL behavior.

```
Episode Start → reset() → Observation
  Step 1: Agent picks wrong farmer → reward 0.0 → try again
  Step 2: Agent picks partial match → reward 0.55 → try again
  Step 3: Agent picks exact senior match → reward 1.0 → done ✅
```
...
├── models.py        # Typed data models (Farmer, Action, StepResult)
...
├── Dockerfile       # Container — Python 3.10, uvicorn
├── requirements.txt
└── README.md
```
...
- **Model:** `Qwen/Qwen2.5-72B-Instruct` via HuggingFace Inference Router
- **API:** OpenAI-compatible client
- **Hardware:** CPU Basic (2 vCPU, 16GB RAM) — within infra limits
- **Inference runtime:** < 2 minutes per episode
- **Max steps per episode:** 3 (configurable)
...
Built for the **Meta × PyTorch OpenEnv Hackathon** organized by Scaler School of Technology.
